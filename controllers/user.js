...
class UserController {
  ...
  static async getAllUsersPage(req, res) {
    ...
    if (search) {
      users = await UserService.searchBy(search, {limit: limit_size, offset}) 
    ...
    res.render('admin/users', {users, currentPage: pageNumber, totalNumberOfPages, totalDocuments, limit_size, offset })
  ...
  static async createUser(req, res) {
    ...
    try {
      ...
      if (req.file) {
        ...
        const imageInfo = await streamUpload(editedImage, process.env.PROJECT_CLOUDINARY_IMAGE_FOLDER + '/admin/users')
        dao.photo = imageInfo.url
        dao.photo_public_id = imageInfo.public_id
  ...
  //   if (dao.password != dao.retype_password) {
  //     req.flash('error_msg', 'Passwords do not match')
  //     return res.redirect('/a/users/new')
  //   }
  //   try {
  //     // check for same username
  //     let sameUsername1 = await UserService.findByUsername(dao.username)
